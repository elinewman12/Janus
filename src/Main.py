...
if __name__ == '__main__':
    ...
    song.load(filename="../MIDI Files/Pop/Billie Eilish/Billie Eilish - Bad Guy  (midi by Carlo Prato) (www.cprato.com).mid", print_file=False)
    # song.get_transition_graph(name="Skyfall")
    # song.change_song_key(Key(tonic=song.detect_key_and_scale()[0:1]), Key())
    # song.get_transition_graph(name="Your Song")
    chain = DynamicMarkovChain("test chain", 8)
    chain.add_song(song)
    song2 = Song()
